...
- [x] **P2 / low-priority cleanup batch** — `/searchresults` with no query no longer spins forever (`SearchResults.jsx`); `CustomersTab` admin set-password now UTF-8-safe hash; `Pricing.jsx` now renders the real plans from `plansConfig` (was a "next phase" placeholder linked sitewide from the footer); welcome email in `BusinessOwnerRegister` now escapes values + sends to the normalized email; `notifyFavoriteCustomers` no longer skips customers whose `is_active` is merely unset; chat `pageContext` now forwarded to `platformHelp`; empty review body no longer renders an empty paragraph in `MyReviewsTab`; stale `claim_disabled` "not in schema" comments corrected (field is in the schema). Build + lint green.

- [x] **Security tactical hardening (SEC-2 + SEC-4)** —
  - **SEC-2 (forgeable claim token):** `sendClaimEmail` now stores a random single-use token (`claim_token`/`claim_token_expiry`/`claim_email`) on the business and links with `&bid=`; `verifyClaimToken` verifies against the stored token + expiry + the email the link was sent to, and consumes it on use; `ClaimBusiness.jsx` passes `bid`. The old reversible/forgeable `btoa(businessId:email:expiry)` token is gone, so anonymous "claim any unclaimed business" is closed. (Old in-flight claim links die → resend.)
  - **SEC-4 (unauthenticated mass-mutation functions):** added a fail-closed guard to the 8 dangerous one-off endpoints (recategorizeBusinesses, backfillSlugs, migrateAllBusinessRatings, migrateImages, migrateWordPressImages, fixImagePaths, fixDoublePathImages, fixBusinessImages) — each now returns 403 unless `ADMIN_TASK_SECRET` (project env) is set AND matched via the `x-admin-secret` header. None are called by the app UI, so this breaks nothing; to run one as a migration, set the env var and pass the header.
  - LIVE-VERIFIED the password flow end-to-end against production: no-token rejected, wrong-token rejected with specific reason, correct token + id-based lookup succeeds, stored hash matches SignIn's, token consumed single-use. Base44 has synced the password fixes — they are live.
  - STILL DEFERRED (needs Base44 dashboard, not code): SEC-3 entity-permission lockdown (the `requiresAuth:false` client can still write entities directly) — requires Base44 RLS config + ideally a real server-side session.

- [x] **Security — tactical hardening (SEC-2 + SEC-4)**:
  - **SEC-2 claim token** — replaced the forgeable/reversible btoa `businessId:email:expiry` claim token with a single-use RANDOM token stored on the business (`claim_token`/`claim_token_expiry`/`claim_email`), verified server-side by `verifyClaimToken` against the stored value + email-sent-to match. Anyone could previously forge a token and claim any unclaimed business (incl. the 233 imports); now impossible. `sendClaimEmail` stores the token + links `?token=&bid=`; `ClaimBusiness.jsx` passes `bid`. (Old in-flight claim links die — resend, like the password flow.)
  - **SEC-4 mass-mutation lockdown** — added a fail-closed `ADMIN_TASK_SECRET` header guard to the 8 publicly-callable one-off migration functions (recategorizeBusinesses, backfillSlugs, migrateAllBusinessRatings, migrateImages, migrateWordPressImages, fixImagePaths, fixDoublePathImages, fixBusinessImages). They were callable by anyone on the internet (mass data corruption + Anthropic-credit/storage abuse). Now 403 unless `ADMIN_TASK_SECRET` env var is set AND passed via `x-admin-secret` header. None are used in normal app flow, so nothing breaks.
  - STILL DEFERRED (needs Base44 dashboard, not code): **SEC-3** — the entity API is open (`requiresAuth:false`), so admin/owner writes (role changes, business edits, approvals) are forgeable from the browser console. Real fix = Base44 entity-level permissions (RLS) + a proper server-side session. This is the big architectural item; tactical code can't close it without breaking the current admin UI.
...
- [x] **FUN-1 (deal date UTC-parsing family)** — new shared `src/lib/dealDates.js` (`parseLocalDate`/`dealStart`/`dealEnd`/`isWithinDealWindow`/`formatDealDate`): date-only strings parsed as LOCAL dates, end date inclusive through end-of-day. Applied to ALL deal sites: active-window checks (`BusinessListing`, `AiSearch`, `SearchResults`, `CategoryListing`, `OverviewTab`, `DealsOverviewTab`, `DealsTab`, `AdminDashboard`), the edit round-trip back-shift + "starts today" email (`DealsTab`), and displays (`DealsSection`, `LatestDeals`, `OverviewTab`, `DealsOverviewTab`, `AdminEditBusinessModal`). Deals now stay active through their stated last day; editing no longer shifts dates back a day. Build clean, no leftover `new Date(deal.*)`.
