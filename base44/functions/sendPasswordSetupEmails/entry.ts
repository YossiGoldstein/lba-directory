...
Deno.serve(async (req) => {
  try {
    ...
    // Get all businesses without password_hash
    const businesses = await base44.asServiceRole.entities.Business.list();
    const businessesWithoutPassword = businesses.filter(b => 
      b.email && !b.password_hash && b.status === 'approved'
    );

    console.log(`Found ${businessesWithoutPassword.length} businesses without passwords`);
    ...
    for (const business of businessesWithoutPassword) {
      try {
        ...
        console.log(`✅ Sent email to ${business.email}`);
      } catch (emailError) {
        failCount++;
        results.push({
          business: business.business_name,
    ...
    return Response.json({
      success: true,
      total: businessesWithoutPassword.length,
      successCount,
      failCount,
      results
    });
